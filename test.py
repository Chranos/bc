...
同时评估:
1. 图文检索性能 (Image-Text Retrieval)
2. 场景分类性能 (Scene Classification)
"""
...
class TestDataset(Dataset):
    ...
    def __init__(self, annotation_file, image_dir, transform=None):
        ...
        for item in self.data:
            if 'file_name' not in item or 'text' not in item or 'scene_category' not in item:
                continue
...
# 图文检索评估
# ============================================
def evaluate_retrieval(image_feats, text_feats, top_k=[1, 5, 10]):
    """计算图文检索指标"""
    print(f"\n" + "="*60)
    print(f"📊 图文检索评估")
    print(f"="*60)
...
def main():
    config = {
        ...
        'results_file': '/workspace/vlm/lab/output/test_results.json',
        'confusion_matrix_file': '/workspace/vlm/lab/output/confusion_matrix.png',
    ...
    print("  - 图文检索 (Image-Text Retrieval)")
    ...
    # 加载数据集
    test_dataset = TestDataset(config['test_file'], config['image_dir'])
    ...
    retrieval_metrics = evaluate_retrieval(
        image_feats, text_feats, top_k=config['top_k']
    )
    ...
    print("="*60)
    print(f"\n📊 关键指标:")
    print(f"  图文检索 Avg R@1:  {retrieval_metrics['avg_R@1']:.2f}%")
    print(f"  图文检索 Avg R@5:  {retrieval_metrics['avg_R@5']:.2f}%")
